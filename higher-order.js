...
let postTaxPrices = prices.map(calculateTax);
...
let orderTotals = orders.map(taxFunc)
...
function onlyBob(num){
  if(purchases.owner == 'Bob'){
    return num += purchases.price;
  }
}
let bobsTotal = purchases.filter(onlyBob)
